...
- `POST /api/xr/triage`
  ...
  - Returns triage result + XR overlay steps + incident timeline.
...
- `docs/unity/RescueSightApiClient.cs`
  ...
  - Handles `SubmitQuest3Triage`, `RefreshOverlay`, and `UpdateIncidentTimeline`
...
5. Render `overlaySteps` as world/head-locked cards and mark actions complete via `UpdateIncidentTimeline`.
6. On app resume/reconnect, call `RefreshOverlay`.
